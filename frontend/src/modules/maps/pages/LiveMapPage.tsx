...
import { MapPin, Search, RefreshCw, Layers, Database } from 'lucide-react';
...
export const LiveMapPage: React.FC = () => {
  ...
  const fetchMapData = async () => {
    try {
      setLoading(true);
      const res = await api.get<{ data: any[] } | any[]>('/analytics/heatmap', {
        params: {
  ...
  const fetchDepartments = async () => {
    try {
      const res = await api.get<{ data: any[] } | any[]>('/departments');
      const data = Array.isArray(res) ? res : (res as any).data || [];
      setDepartments(data);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1 font-sans">
            Real-time geospatial visualization powered by spatial coordinates and interactive Leaflet GIS cluster rendering.
          </p>
        </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-1 flex items-center space-x-1 text-xs">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-semibold transition-all ${
                viewMode === 'markers' ? 'bg-cyan-500 text-slate-950 shadow-md' : 'text-slate-400 hover:text-white'
              }`}
            >
              Markers Map
            </button>
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-semibold transition-all ${
                viewMode === 'clusters' ? 'bg-cyan-500 text-slate-950 shadow-md' : 'text-slate-400 hover:text-white'
              }`}
            >
              Cluster Heatmap
            </button>
      ...
      <div className="bg-slate-900/90 backdrop-blur-md p-4 rounded-xl border border-slate-800 flex flex-col lg:flex-row items-center justify-between gap-4">
        <div className="relative w-full lg:w-72">
          ...
          <input
            ...
            className="w-full bg-slate-950 border border-slate-800 rounded-lg pl-9 pr-4 py-2 text-xs text-white focus:outline-none focus:border-cyan-500"
          />
        </div>
      ...
      <div className="bg-slate-900/90 backdrop-blur-md p-4 rounded-2xl border border-slate-800 relative">
        {loading && (
