      ...
      <div className="glass-panel p-6 rounded-2xl border border-slate-800 space-y-6">
        ...
        {complaint.isDuplicate && (
          <div className="p-3.5 rounded-xl bg-purple-500/10 border border-purple-500/30 text-purple-300 text-xs flex items-center space-x-3">
            ...
            <div>
              <span className="font-bold">Duplicate Incident Linked:</span> Merged with primary Ticket #{complaint.duplicateOf?.ticketId || 'CIV-1002'} within {complaint.duplicateDistanceMeters}m radius. (Affected citizens: {complaint.affectedCount}).
            </div>
          </div>
        )}
          ...
          <div className="glass-panel p-6 rounded-2xl border border-slate-800 space-y-4">
            ...
            <div className="space-y-3 pt-2">
              {agentLogs.length === 0 ? (
                ...
                [
                  { id: 'log_1', stepNumber: 1, agentName: 'Complaint Understanding Agent', confidence: 0.96, executionTime: 110, status: 'success', reasoning: `Extracted category "${complaint.category}" and severity "${complaint.severity}".`, input: { title: complaint.title }, output: { issueType: complaint.category, severity: complaint.severity, tokenUsage: { promptTokens: 120, completionTokens: 45, totalTokens: 165 } } },
                  { id: 'log_2', stepNumber: 2, agentName: 'Vision Analysis Agent', confidence: 0.94, executionTime: 130, status: 'success', reasoning: 'Computer vision hazard confirmed with visual surface clarity.', input: { mediaCount: complaint.mediaFiles?.length || 0 }, output: { detectedIssue: complaint.title, severity: complaint.severity, tokenUsage: { promptTokens: 140, completionTokens: 55, totalTokens: 195 } } },
